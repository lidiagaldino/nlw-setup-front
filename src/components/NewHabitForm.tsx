...
const NewHabitForm = () => {
    ...
    return (
        <form onSubmit={createNewHabit} className='w-full flex flex-col mt-6'>
            ...
            <input value={title} onChange={event => setTitle(event.target.value)} className='p-4 rounded-lg mt-3 bg-zinc-800 text-white placeholder:text-zinc-400' type='text' id='title' placeholder='ex.: Exercicios, dormir bem, etc...' autoFocus />
            ...
            <div className='flex flex-col gap-2 mt-3'>
                {availableWeekDays.map((weekDay, index) => {
                    return (
                        <Checkbox.Root checked={weekDays.includes(index)} key={weekDay} className='flex items-center gap-3 group' onCheckedChange={() => handleToggleWeekDay(index)}>
